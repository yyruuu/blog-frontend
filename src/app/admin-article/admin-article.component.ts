...
export class AdminArticleComponent implements OnInit {
  ...
  async getArticles() {
    const res = await this.http.get(`${this.configService.path}/api/article?offset=${(this.page-1)*this.pageNation["pageSize"]}&pageSize=${this.pageNation["pageSize"]}&sort=${JSON.stringify([this.sortBy.name, this.sortBy.type])}&filter=null`).toPromise();
    console.log(res);
    if (res["err"] === 0) {
